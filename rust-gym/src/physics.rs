...
//!
//! Player box is 0.6 wide x 1.8 tall, `pos` is the feet centre. Movement follows vanilla's
//! moveRelative -> collide -> gravity/friction ordering with the standard constants. Collision
//! is the classic axis-separated sweep (resolve Y, then X, then Z) against real block collision
//! shapes from `pumpkin-data`. Horizontal-speed constants are vanilla-derived but not yet
//! tuned/validated tick-for-tick against the client; the collision + gravity + landing path is.
...
impl Aabb {
    ...
    /// Do the two boxes overlap on the two axes other than `ax`?
...
pub struct Agent {
    ...
    /// Block currently being mined and accumulated break progress [0,1).
    pub mine_target: Option<[i32; 3]>,
    pub mine_progress: f64,
    /// Persistent home (respawn point on death) and facing.
    pub home: [f64; 3],
    pub home_yaw: f32,
    /// Stuck tracking: farthest-from reference and the last tick real progress was made.
    pub ref_xz: [f64; 2],
...
impl Agent {
    ...
    /// Add one of `item_id` to the inventory: stack onto a matching non-full slot, else the
    /// first empty slot. Returns false if the inventory is full.
    pub fn add_item(&mut self, item_id: i32) -> bool {
        for s in 0..INV_SLOTS {
            if self.inv_item_id[s] == item_id && self.inv_count[s] < 64 {
                self.inv_count[s] += 1;
                return true;
            }
        }
    ...
    pub fn count_item(&self, item_id: i32) -> u32 {
        (0..INV_SLOTS)
            .filter(|&s| self.inv_item_id[s] == item_id)
            .map(|s| u32::from(self.inv_count[s]))
            .sum()
    }

    /// Block coordinates of the agent's feet (the voxel-grid centre).
    ...
    /// Eye position (vanilla eye height 1.62).
    pub fn eye(&self) -> [f64; 3] {
        [self.pos[0], self.pos[1] + 1.62, self.pos[2]]
    }

    /// Advance one tick: camera, movement input, jump, collision, gravity, friction.
    pub fn step(&mut self, world: &World, act: &Action) {
        ...
        let friction = if self.on_ground {
            DEFAULT_SLIP * AIR_FRICTION
        } else {
            AIR_FRICTION
        };
        let mut accel = if self.on_ground {
            WALK_SPEED * (0.216 / (friction * friction * friction))
        } else {
            0.02
        };
        if act.sprint != 0 {
    ...
    fn move_relative(&mut self, amount: f64, forward: f64, strafe: f64) {
        ...
        let scale = if d2 > 1.0 { amount / d2.sqrt() } else { amount };
    ...
    /// Sweep the player box by `vel` against block colliders, resolving Y, then X, then Z.
    ...
    /// All block collision AABBs overlapping the broadphase region (box swept by `vel`).
