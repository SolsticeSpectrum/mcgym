//! Canonical observation/action byte layout — the cross-language contract.
//!
//! Mirrors `schema/mcai_schema.yaml` (schema_version 1) and the NumPy dtype built
//! in `trainer/mcai_train/schema/spec.py`. Records are packed little-endian with
//! NO field alignment: fields are written sequentially in declared order, so e.g.
//! the i64 `tick` lands at byte offset 4, not an 8-aligned offset. Do not model
//! these as `#[repr(C)]` structs — that would insert padding and break the contract.
//!
//! Correctness is pinned by the golden fixtures in `schema/fixtures/` (see tests).
...
/// One agent's observation. `Vec`/array fields default to zero-filled.
...
/// One agent's action, as written by the trainer into the ACTION region.
...
/// Sequential little-endian writer over a fixed slice. Panics on overrun, which
/// would mean the layout drifted from `OBS_NBYTES`/`ACTION_NBYTES`.
struct LeWriter<'a> {
    buf: &'a mut [u8],
    pos: usize,
}
...
/// Sequential little-endian reader (used for round-trip tests and reading actions).
struct LeReader<'a> {
    buf: &'a [u8],
    pos: usize,
}
...
impl Obs {
    /// Write this observation into `dst` (must be exactly `OBS_NBYTES`).
    pub fn encode_into(&self, dst: &mut [u8]) {
        ...
        for &v in &self.voxel_blocks {
            w.i32(v);
        }
        for &v in &self.voxel_far {
            w.i32(v);
        }
        ...
        for v in self.entity_type_id {
            w.i32(v);
        }
        for row in self.entity_rel_pos {
            for v in row {
                w.f32(v);
            }
        }
        for row in self.entity_vel {
            for v in row {
                w.f32(v);
            }
        }
        for v in self.entity_yaw {
            w.f32(v);
        }
        for v in self.entity_health {
            w.f32(v);
        }
        for v in self.entity_flags {
            w.u8(v);
        }
        for v in self.inv_item_id {
            w.i32(v);
        }
        for v in self.inv_count {
            w.u8(v);
        }
    ...
    /// Parse an observation from exactly `OBS_NBYTES` (round-trip / debugging).
    pub fn decode(src: &[u8]) -> Self {
        ...
        for c in o.voxel_blocks.iter_mut() {
            *c = r.i32();
        }
        for c in o.voxel_far.iter_mut() {
            *c = r.i32();
        }
        ...
        for c in o.entity_type_id.iter_mut() {
            *c = r.i32();
        }
        for row in o.entity_rel_pos.iter_mut() {
            *row = [r.f32(), r.f32(), r.f32()];
        }
        for row in o.entity_vel.iter_mut() {
            *row = [r.f32(), r.f32(), r.f32()];
        }
        for c in o.entity_yaw.iter_mut() {
            *c = r.f32();
        }
        for c in o.entity_health.iter_mut() {
            *c = r.f32();
        }
        for c in o.entity_flags.iter_mut() {
            *c = r.u8();
        }
        for c in o.inv_item_id.iter_mut() {
            *c = r.i32();
        }
        for c in o.inv_count.iter_mut() {
            *c = r.u8();
        }
...
impl Action {
    /// Parse an action from exactly `ACTION_NBYTES`.
    pub fn decode(src: &[u8]) -> Self {
